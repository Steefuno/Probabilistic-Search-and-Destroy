...
class Agent:
    ...
    def search(self):
        ...
        if self.env.query(self.position):
            return True
        # If target is not found, try again then either return True or update beliefs and return False
        else:
            # Query the cell a second time
            if self.env.query(self.position):
                return True
            else:
                cell_data = self.cells[self.position]
                fail_probability = cell_data["fail_probability"]
                # If the environment is the forest or caves, the cell will be queried two more times (if necessary)
